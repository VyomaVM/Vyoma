...
- `.deb`/`.rpm` packaging with bundled Cloud Hypervisor binary
...
│   │       ├── cloud-hypervisor.rs  # Cloud Hypervisor API wrapper
...
│   │       ├── vmm.rs          # Cloud Hypervisor HTTP client
...
│   └── rpm/
├── scripts/
├── bin/                        # Bundled Cloud Hypervisor binary
└── Cargo.toml
```
...
```
~/.vyoma/images/<image-hash>/
    rootfs.ext4       # Read-only base image
    vyoma-config.json  # NEW: parsed OCI config
    manifest.json
```

**Step 3**: In `crates/vyomad/src/vm_manager.rs`, before calling the Cloud Hypervisor boot API, generate `/sbin/vyoma-init` inside the VM's rootfs COW layer and set it as the kernel `init=` boot parameter.
...
**Step 4**: Pass `init=/sbin/vyoma-init` in the kernel boot arguments when launching Cloud Hypervisor.
...
Add the static `virtiofsd` binary to `bin/` alongside `cloud-hypervisor`. The packaging scripts must include it in the `.deb`/`.rpm` at `/usr/lib/vyoma/virtiofsd`.
...
Documentation=https://github.com/Subeshrock/vyoma
...
#   CAP_SETUID     — for jailer subprocess (Cloud Hypervisor)
...
chown root:vyoma /run/vyoma/vyoma.sock 2>/dev/null || true
chmod 0660 /run/vyoma/vyoma.sock 2>/dev/null || true
...
pub struct RecoveryResult {
    ...
    pub orphaned: Vec<String>,  // VMs whose Cloud Hypervisor process is gone
    ...
    for state in wal.all_states()? {
        match state.status {
            VmStatus::Running { pid, fc_socket, ip } => {
                // Check if the Cloud Hypervisor process is still alive
                if process_exists(pid) {
                    // Re-adopt the running VM — reconnect to Cloud Hypervisor socket
...
pub async fn destroy_vm(&self, vm_id: &str) -> Result<()> {
    ...
    // 8. Remove Cloud Hypervisor socket and working directory
    if let Some(fc_dir) = &state.cloud-hypervisor_dir {
...
pub async fn commit_vm(vm_id: &str, tag: &str, vm_manager: &VmManager) -> Result<ImageId> {
    // 1. Pause VM via Cloud Hypervisor API
...
pub struct SnapshotNode {
    ...
    pub memory_path: PathBuf,       // Cloud Hypervisor .mem file
    pub snapshot_path: PathBuf,     // Cloud Hypervisor .snap file
...
pub struct MigrationSender {
    vm_fd: VmFd,
    fc_client: Cloud HypervisorClient,
...
impl MigrationSender {
    pub async fn migrate(
        ...
        // Send Cloud Hypervisor snapshot (CPU state)
...
pub async fn hibernate_vm(&self, vm_id: &str) -> Result<HibernationInfo> {
    ...
    // 1. Cloud Hypervisor snapshot (CPU + memory state to files)
    ...
    // 2. Stop the Cloud Hypervisor process (releases vCPUs and memory)
...
pub async fn resume_vm_from_hibernation(&self, vm_id: &str) -> Result<()> {
    ...
    // 2. Start new Cloud Hypervisor process
    let fc_socket = new_fc_socket_path(vm_id);
    let fc_process = spawn_cloud-hypervisor(vm_id, &fc_socket)?;
    
    // 3. Load snapshot (Cloud Hypervisor resumes from exact state)
...
import (
    ...
    vyoma "github.com/Subeshrock/vyoma/sdk/go"
)
...
/// Svyoma a VMIF manifest with an Ed25519 key
...
import (
    ...
    pb "github.com/Subeshrock/vyoma/vyoma-proto/gen/go"
)
...
### 7.3 Mock Cloud Hypervisor Server

For unit tests that test vyomad behavior without KVM:

```rust
// tests/mocks/cloud-hypervisor_server.rs

pub struct MockCloud Hypervisor {
    addr: SocketAddr,
    state: Arc<Mutex<MockVmState>>,
}

impl MockCloud Hypervisor {
...
/usr/lib/vyoma/cloud-hypervisor   # Bundled Cloud Hypervisor VMM
