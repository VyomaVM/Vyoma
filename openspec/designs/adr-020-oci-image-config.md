...
1. **Extraction**: During `vyoma pull` and `vyoma build`, the `OciManager` will parse the layer metadata and emit an `vyoma-config.json` file inside the `.vyoma/images/<hash>/` directory alongside the `rootfs.ext4`.
2. **Boot Injection**: When `vyoma run` is invoked, `vyomad` (the daemon) will dynamically generate a shell script (`/sbin/vyoma-init`).
...
4. **Kernel Cmdline**: Firecracker will be booted with `init=/sbin/vyoma-init`. The script will export the environment variables, `cd` into the predefined WorkingDir, and `exec` the combined ENTRYPOINT + CMD array.
...
- Handling shell escaping for complex CMD arrays inside the `/sbin/vyoma-init` script must be extremely robust to avoid injection errors or malformed parsing.

## Diagram / Flow
```mermaid
sequenceDiagram
    participant CLI as vyoma pull/run
    participant OCI as OciManager (vyoma-core)
    participant Daemon as vyomad (VM Manager)
    participant FC as Firecracker
    
    CLI->>OCI: pull_image(nginx:alpine)
    OCI->>OCI: Download layers + config blob
    OCI-->>Daemon: Save rootfs.ext4 + vyoma-config.json
    CLI->>Daemon: run_vm(nginx:alpine)
    Daemon<<->>Daemon: generate /sbin/vyoma-init
    Daemon->>Daemon: debugfs write init to VM CoW layer
    Daemon->>FC: boot(init=/sbin/vyoma-init)
    FC-->>CLI: VM Running
```
