# ADR-035: VMIF Image Format - vyoma-image Crate

**Status**: Accepted | Phase 3.6 (v1.6)

## Summary
Implement VMIF (VM Image Format) - the stable on-disk format for Vyoma images with Docker Hub bridge support.

## Context
As part of Phase 3, we need to implement a proper image format for Vyoma. Currently images are raw OCI converted at runtime every pull. VMIF provides:
...
### Crate Structure
- `crates/vyoma-image/src/vmif.rs` - VMIF struct and manifest
- `crates/vyoma-image/src/hub_bridge.rs` - Docker Hub → VMIF conversion
- `crates/vyoma-image/src/signing.rs` - Image signing (future)
