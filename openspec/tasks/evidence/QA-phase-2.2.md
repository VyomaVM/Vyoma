...
- [x] Verify vyoma-net crate is created with proper structure
...
## Checks Performed
1. **Crate structure**: Confirmed `crates/vyoma-net/` with Cargo.toml, src/lib.rs, bridge.rs, tap.rs, error.rs
2. **API design**: BridgeManager and TapManager structs with async methods
3. **Compilation**: `cargo check -p vyoma-net` passed (with warnings)
