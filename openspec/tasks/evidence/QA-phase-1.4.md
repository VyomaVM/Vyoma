...
- [x] Verify sled WAL opens at `~/.ignite/state/ignite.db`
- [x] Check WAL entries log on VM create/start/stop
- [x] Verify recovery scans `~/.ignite/vms/` on startup
- [x] Ensure fsync after every WAL append for durability

## Checks Performed
1. **WAL initialization**: Config path correctly set to `home.join(".ignite").join("state")`
...
6. **Build check**: `cargo check -p ignited` passed
