...
2. **Networking**: Rootless networking (slirp4netns) prevents standard Overlay/VXLAN implementation for Clustering (Ignite Swarm).
...
- **Daemon (`ignited`)**: Runs as `root` (managed by systemd). This grants access to KVM, Device Mapper, and Kernel Networking (Bridges/VXLAN).
- **CLI (`ign`)**: Runs as an unprivileged user. It communicates with the daemon via a Unix Socket (`/var/run/ignite.sock`).
