use anyhow::Result;
use clap::Parser;
use ignite_agent_vm::{collect_metrics, collect_process_list, read_file_content, execute_command, AgentRequest, AgentResponse};
...
async fn run_tcp(port: u16) -> Result<()> {
    ...
    info!("ignite-agent-vm listening on tcp:{}", port);
...
mod tests {
    ...
    fn test_cli_defaults() {
        let opts = Opts::parse_from(&["ignite-agent-vm"]);
    ...
    fn test_cli_custom_port() {
        let opts = Opts::parse_from(&["ignite-agent-vm", "--port", "8080"]);
